...
extern Buzzer_control_config_t *Buzzer_control_config;

extern Fan_control_config_t *Fan_control_config;
...
// Used for returning the ledrgb configuration
...
static void wifi_app_task(void *pvParameters)
{
	...
	initialize_ledRGB_config();
	initialize_Buzzer_config();
	initialize_Fan_config();
	...
	/*
				...
				case WIFI_APP_MSG_START_HTTP_SERVER:
					...
					xTaskCreatePinnedToCore(RGBControl_task, "RGBControl_task", 2048, NULL, 2, NULL, 0);
					xTaskCreatePinnedToCore(BuzzerControl_task, "BuzzerControl_task", 2048, NULL, 3, NULL, 1);
					xTaskCreatePinnedToCore(FanControl_task, "FanControl_task", 2048, NULL, 3, NULL, 1);
					http_server_start();
...
void wifi_app_start(void)
{
	...
	// Allocate memory for the ledrgb configuration
	ledRGB_control_config = (ledRGB_control_config_t *)malloc(sizeof(ledRGB_control_config_t));
	memset(ledRGB_control_config, 0x00, sizeof(ledRGB_control_config_t));

	// Allocate memory for the buzzer configuration
	Buzzer_control_config = (Buzzer_control_config_t *)malloc(sizeof(Buzzer_control_config_t));
	memset(Buzzer_control_config, 0x00, sizeof(Buzzer_control_config_t));

	// Allocate memory for the buzzer configuration
	Fan_control_config = (Fan_control_config_t *)malloc(sizeof(Fan_control_config_t));
	...
	//xTaskCreatePinnedToCore(RGBControl_task, "RGBControl_task", 2048, NULL, 3, NULL, 1);
